...
a1.start = function(hookElementSelection, dataurlForJsonFile) {
	...
	a1.suppliers = {};
	a1.bakedRecipes = [];
	//creating a div for loading and appending
	var loadingdiv = $("<div id='loading'></div>");
	loadingdiv.append('<h4>LOADING</h4>');
	hookElementSelection.append(loadingdiv);
	//callback function for fading in and out the loading div
	loadingdiv.fadeIn(1000,function(){
	...
	$.ajax({url:dataurlForJsonFile}).success(function(data) {
		...
		//appending the logo and the image	
		var logo = $("<div id='logo'></div>");
		logo.append('<h4 id="cakeTitle">CAKE BABY</h4>');
		logo.append('<img id="cake" src="images/cakeBaby.jpg"/>');
		hookElementSelection.append(logo);
		//get the recipe data
		parseJSONData(data);
...
var renderCalculator = function(base) {
	var costHTML = $("<div id='buttondiv'><button id='total'>Calculate Weekly Total</button></div>");
	...
	costHTML.on("click", function(event) {
		...
		//sliding up the recipe list
		$(".recipe").slideUp(1000,function(){
		//hiding the button	
		$("#total").hide();	
		//displaying the results of the supplier cost and total first for the user by sliding down
		$("#results").delay(100).slideDown(1000,function(){	
		//displaying the ingredient amount required
		$(".grid").delay(100).slideDown(1000);
		});
		});
	});
...
var splitTotalsAndPrint = function(base, ingredientTotals)
{
	...
	for (supplierName in supplierTotals) {
		...
		var finalHTML = $("<div class='final'></div>");
		var supplierHTML = $("<h4>" + supplierName + "</h4>");
		var supplierTotalHTML = $("<h4><span> Supplier Cost: $" + supplierTotalCost.toFixed(2) + "</span></h4>");
...
a1.Product = function(name,cost,per,supplier) {
	...
	this.render = function(base, amount) {
		var nameHTML = $("<h4>" + amount+ " " + this.per + " " +  this.name + "</h4>");
		var costHTML = $("<span>($" + this.cost + " per " + this.per + ")</span>");
		var supplierHTML = $("<div></div>");
		this.supplier.render(supplierHTML);
		//creating a div for the purpose of alignment	
		var finalHTML = $("<div class='grid'></div>");
...
a1.Supplier = function(name) {
	this.name = name;
	this.render = function(base) {
		var nameHTML = $("<h4  class='supplier'>" + this.name + "</h4>");
